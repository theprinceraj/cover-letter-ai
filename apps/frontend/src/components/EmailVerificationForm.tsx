...
export const EmailVerificationForm = () => {
    ...
    const { user, setIsEmailVerificationModalOpen, refreshAuth, fetchWithAuth } = useContext(AuthContext)!;
    ...
    const handleResendCode = useCallback(async () => {
        ...
        try {
            const response = await fetchWithAuth<AuthService_VerificationEmail_Response>({
    ...
    return (
        <div className="max-w-md mx-auto p-8 bg-slate-900/50 backdrop-blur-sm border border-slate-700 rounded-xl shadow-2xl">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <Button type="submit" fullWidth isLoading={isLoading} disabled={otp.length !== 6 || isLoading}>
                    Verify Email
                </Button>
