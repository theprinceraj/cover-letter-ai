...
export const Spinner: React.FC<SpinnerProps> = ({ size = "md", variant = "default", message, className = "" }) => {
    const sizeClasses = {
        sm: "size-4",
        md: "size-8",
        lg: "size-12",
    ...
    const spinnerContent = (
        <div className={`flex flex-col items-center justify-center ${className}`}>
            <div className="relative">
                {/* Outer ring */}
                <div className={`${sizeClasses[size]} border-2 border-secondary-600 rounded-full animate-pulse`} />
                {/* Spinning ring */}
                <div
            ...
            {message && <p className="mt-4 text-white text-center font-medium animate-pulse">{message}</p>}
        </div>
    );
